...
def custom_score(originals, paraphrases):
    ...
    for original, paraphrase in zip(originals, paraphrases):
        ...
        original_emb = model(tokenizer.encode(original, padding=True, truncation=True, max_length=128, return_tensors='pt'))[0][0]
        paraphrase_emb = model(tokenizer.encode(paraphrase, padding=True, truncation=True, max_length=128, return_tensors='pt'))[0][0]
